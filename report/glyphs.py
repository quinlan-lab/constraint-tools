...
def add_exon_glyph(plot_params, plot, exons, direction):
    ...
    def adjust_arrow_coords(coords, direction):
        ...
        while (max(ret) - min(ret)) / plot_params['plot_width'] > 0.01:
            ret = [r * .9 for r in ret]
    ...
    for i in range(len(features_compact)):
        ...
        ys_dict.append([{'exterior': [plot_params['transcript_height'] - plot_params['exon_height'] / 2,
                                      ...
                                      plot_params['transcript_height']], 'holes': []}])  # triangle: prev [7,13,10]
...
def add_variant_glyph(plot_params, variant_params, plot, variant_ls, height=9, radius=5,
                      line_width=2):  # height prev 18
    ...
    def get_y1(fn, ls):
        ...
        y1_circle = [c * (plot_params['plot_height'] - plot_params['y0'] - variant_params[
            'min_lollipop_height'] - radius - line_width - 2) / max([c for c in ls_fn]) + plot_params['y0'] +
                     variant_params['min_lollipop_height'] for c in ls_fn]
        y1_segment = [y - radius for y in y1_circle]
    ...
    source = ColumnDataSource(dict(x=x, r=r, y0=y0s, y1_circle=y1_ci_li_ct, y1_segment=y1_sg_li_ct,
