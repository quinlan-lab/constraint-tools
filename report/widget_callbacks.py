...
def add_legend(user_line_params, height=70, width=270):
    ...
    legend_plot = figure(plot_height=30*num_lines,plot_width=width,toolbar_location=None)
    ...
    for axis_name in user_line_params:
        ...
        #TODO add alpha?
            legend_glyph = legend_plot.line(x=[0,1],y=[0,1],line_color=user_line_params[axis_name]['lines'][line]['color'])
            legend_items.append((line, [legend_glyph]))
            legend_glyph.visible = False

    legend = Legend(items=legend_items, location=(0,0))
    legend_plot.add_layout(legend, 'center')
    legend_plot.legend.label_text_font_size = '12px'
