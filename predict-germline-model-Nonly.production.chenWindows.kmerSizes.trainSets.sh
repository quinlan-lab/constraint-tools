...
# for kmer_size in "1" "3" "5" "7"; do 
for kmer_size in "5"; do 
  # for train_set_label in "noncoding" "coding" "chenWindows"; do
  for train_set_label in "noncoding"; do
    # for noisy_label in ".noisified" ""; do
    for noisy_label in ".noisified"; do
      log_file="${CONSTRAINT_TOOLS_DATA}/genome-wide-predictions/predict-germline-grch38-Nonly.chenWindows.kmerSize-${kmer_size}.trainSet-${train_set_label}${noisy_label}.log"
      job_name="predict-germline-model-Nonly.production.chenWindows.kmerSize-${kmer_size}.trainSet-${train_set_label}${noisy_label}"
      sbatch \
        --wait \
        --output=${log_file} \
        --job-name=${job_name} \
