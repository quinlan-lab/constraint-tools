...
percentage_of_island_that_is_CpG="8"
percentage_of_island_that_is_C_or_G="9"
observed_CpG_number_relative_to_expectation="10"

filter_and_sort () {
  local sort_column=$1
  zcat ${CpGs} |
    awk -F $'\t' -v CpG_island_length=${CpG_island_length} '$CpG_island_length > 1000' |
    bedtools intersect -a - -b ${neutral_regions} -wa -f 0.5 -u |
    sort --field-separator=$'\t' -k${sort_column},${sort_column}n
}

filter_and_sort ${percentage_of_island_that_is_CpG}
