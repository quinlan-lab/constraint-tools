...
LABELED_ENHANCERS="${CONSTRAINT_TOOLS_DATA}/khurana/labeled-enhancers.${VERSION}.bed"
...
create-header () {
  echo -e "$(get-labeled-enhancers-head)\t$(get-topmed-deletions-head)\tenhancer-deletion-overlap"
}

intersect-enhancers-with-deletions-with-header () {
  enhancers_intersect_deletions="${CONSTRAINT_TOOLS_DATA}/khurana/labeled-enhancers-${VERSION}-intersect-topmed-deletions.bed"
  (
    create-header
    intersect-enhancers-with-deletions
  ) > ${enhancers_intersect_deletions}  
  info "Wrote labeled enhancers with intersecting topmed deletions to:" ${enhancers_intersect_deletions}  
}
