...
get-train-khurana-enhancers () {
  local prefix=$1
  local khurana_enhancers="${CONSTRAINT_TOOLS_DATA}/khurana/${prefix}.hg38.hg19.sorted.bed"
...
intersect-enhancers-with-windows () {
  ...
  bedtools intersect \
      -a <(get-train-khurana-enhancers ${prefix}) \
      -b <(get-chen-windows) \
      -wa \
      -wb \
...
intersect-enhancers-with-windows "lof-tolerant-enhancers"
intersect-enhancers-with-windows "low-lof-tolerance-enhancers"
