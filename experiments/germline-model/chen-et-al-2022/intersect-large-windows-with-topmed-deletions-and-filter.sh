...
intersect-large-windows-with-deletions-and-filter () {
  large_windows_intersect_deletions="${CONSTRAINT_TOOLS_DATA}/benchmark-genome-wide-predictions/chen-et-al-2022/large-windows-with-deletion-counts.bed"
  bedtools subtract \
    -a <(intersect-large-windows-with-deletions) \
    -b <(get-exclude-regions) \
    -A \
    > ${large_windows_intersect_deletions}
  info "Wrote large windows with intersecting topmed deletions to:" ${large_windows_intersect_deletions}  
}
