...
work_directory="work-train-germline-model-Nonly"
...
train_on_subset_of_trustworthy_noncoding_regions () {
  ...
  local model="${CONSTRAINT_TOOLS}/tests/germline-model/model-germline-grch38-Nonly-${number_of_trustworthy_noncoding_regions}.json" 
  ...
  constraint-tools train-germline-model-Nonly \
    ...
    --kmer-size ${kmer_size} \
    --train-regions <(fetch_subset_of_trustworthy_noncoding_regions | bgzip) \
    --model ${model} \
    --work ${work} \
...
if [[ ${all_trustworthy_noncoding_regions} == "true" ]]; then
  error "training on all trustworthy noncoding regions not implemented"
else
  train_on_subset_of_trustworthy_noncoding_regions
fi 
